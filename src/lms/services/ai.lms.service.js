...
async function generateGoalRecommendations(query) {
  const prompt = `Based on the partial search query "${query}", suggest 5 professional job titles or career goals.
  ...
  try {
    ...
    if (openai) {
     ...
     console.error('generateGoalRecommendations failed:', error.message);
  }
  return [];
}
